...
```{r}
ruta_archivo <- "Sesion 3/resultados/grafico1.PNG"
ruta_archivo
png(ruta_archivo)
...
`ggplot` tiene __muchos__ comandos, y no tiene sentido saberlos de memoria, es siempre útil reutilizar gráficos viejos y tener a mano el [link](https://www.rstudio.com/wp-content/uploads/2016/11/ggplot2-cheatsheet-2.1.pdf).    
...
+ Cuando queremos simplemente mejorar el diseño, se asigna por fuera, o dentro de cada tipo de gráficos, ```geom_col(color = 'green')```.
...
La librería GGplot tiene a su vez muchas otras librerías que extienden sus potencialidades. Entre las mas usadas están:
...
```{r strip.white=FALSE}
library(data.table)
Individual_t119 <- fread("data/usu_individual_t119.txt")
```
