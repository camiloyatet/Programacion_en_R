...
A lo largo de este notebook, se trabajara con el paquete [__tidyverse__](https://www.tidyverse.org/). El mismo agrupa una serie de paquetes que tienen una misma lógica en su diseño y por ende funcionan en armonía.     
Entre ellos, usaremos principalmente __dplyr__ y __tidyr__ para realizar transformaciones sobre nuestro set de datos.
      
A continuación cargamos la librería a nuestro ambiente. Para ello debe estar previamente instalada.
```{r echo=TRUE, message=FALSE, warning=FALSE}
library(tidyverse)
```

Para mostrar el funcionamiento básico de tidyverse utilizaremos a modo de ejemplo datos inventados del mercado laboral
...
El caracter principal para utilizar este paquete es ```%>%``` , _pipe_ .   
...
Para ello crearemos previamente un Dataframe que contenga las cantidades de población total y población económicamente activa para cada uno de los períodos de _Datos_.
...
El paquete tidyr está pensado para facilitar la limpieza de los datos.
...
Se usuaran los datos de la librería `gapminder`.
